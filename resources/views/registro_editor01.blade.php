                  ...
                  <div class="modal-body">
                    <div class="panel-body" >
                    	<div class="row" style="padding-left: 25px;">
                            ...
                            <div class="row">	
                                <div class="col-lg-3 col-md-3 col-xs-3">
                                    <p class="arealabel"> Técnica Utilizada </p>
                                </div>
                                <div class="col-lg-1 col-md-1 col-xs-1">
                                  <p class="arealabel">  : </p>
                                </div>
                            ...
                            <div class="row hidden" id="otratecnica">	
                                ...
                                <div class="col-lg-8 col-md-8 col-xs-8">  
                                 <input id="tecnicareporteotra" class="form-control" placeholder="Indique técnica utilizada">
                                </div>
                            </div>
                            <div class="row">	
                                <div class="col-lg-3 col-md-3 col-xs-3">
...
<script>
	...
	function tablasverreporte(idmedicion){
			...
			$('#modalloading').modal({backdrop: 'static', keyboard: false});
			$.ajax({
					url: '{{ Route("registro.fan.reporte") }}',
					type: 'get',
					data: {		
			...
			$.ajax({
					url: '{{ Route("registro.load.pambientales.reporte") }}',
					type: 'get',
					data: {
						_token: "{{ csrf_token() }}",		
	...
	function eliminar_doc(id){
		var msg1 = "Eliminar";
		var msg2 = "¿Está seguro que desea eliminar el archivo?";
		swal({
				  title: msg1,
				  text: msg2,
				  type: "warning",
	...
	function existefechamuestreo(){
		if($('#fechareporte').val() != "" && $('#timereporte').val() != "" && $('#fechaanalisis').val() != "" && $('#timeanalisis').val() != ""){
			$.ajax({
					url: '{{ Route("registro.existe.fecha.muestreo") }}',
					type: 'post',
					data: {
						_token: "{{ csrf_token() }}",
	...
	function guardarreporte(tecnica,iddiato,diato0,diato1,diato2,diato3,diato4,diato5,diato6,iddino,dino0,dino1,dino2,dino3,dino4,dino5,dino6,idoesp,oesp0,oesp1,oesp2,oesp3,oesp4,oesp5,oesp6,idpamb,pamb0,pamb1,pamb2,pamb3,pamb4,pamb5,pamb6,idpambo,pambo0,idmortalidad,fechamedicion,fechaanalisis,guardar){
		...
		$('#modalloading').modal({backdrop: 'static', keyboard: false});
		$.ajax({
				url: '{{ Route("registro.save.registro") }}',
				type: 'post',
				data: {
				 _token: "{{ csrf_token() }}",
				 Tecnica: 	   						tecnica,
	...
	function savearchivo(form_data,Alarma,Comentario,Concentracion,Nocivo,Nocivo_P,Comentario_Precaucion,Concentracion_Precaucion,Mortalidad,IDcentro,Nombre_Centro,fechamedicion,idmed,guardar){
		...
		$('#modalloading').modal({backdrop: 'static', keyboard: false});
		$.ajax({
				url: '{{ Route("registro.save.archivo.registro") }}',
				dataType: 'text', 
				cache: false,
				contentType: false,
				processData: false,
...
//			
				...
				$('#modalloading').modal({backdrop: 'static', keyboard: false});
				$.ajax({
					url: '{{ route("registro.destinatario.alarma") }}',
					type: 'post',
					data: {
					 _token: "{{ csrf_token() }}",
					 Alarma:	 Alarma,
					...
					{	
						...
						if(msg2['Error'] == 0 && msg2['Destinatarios'].length > 0){
							$.ajax({
								url: '{{ Route("registro.send.alarma") }}',
								type: 'post',
								data: {
								 _token: "{{ csrf_token() }}",
								 Alarma:	 		Alarma,
...
//			WinPrint.document.write(prtContent.innerHTML);
	...
	function loaddatagrafico(){
		...
		//if(especie_grafico_1 != "" || especie_grafico_2 != "" || especie_grafico_3 != ""){
			$('#modalloading').modal({backdrop: 'static', keyboard: false});
			$.ajax({
				url: '{{ Route("registro.load.historial.centro.pdf") }}',
				type: 'get',
				dataType: 'json',
				data: {
					 _token: "{{ csrf_token() }}",

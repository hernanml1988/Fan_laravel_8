...
//--------------------------------------------------------------------
	...
	if(  $userid == 1 || $userid == 88 || $userid == 275 || $userid == 235 || $userid == 81 || $userid == 313 || $userid == 78 ||
	$userid == 55 || $userid == 80 ||
	//Camanchaca
	$userid == 266 || $userid == 83 || $userid == 583 || $userid == 110 ||
	//$userid == 82 || $userid == 110 || $userid == 83 ||
	//Australis
	$userid == 138 || $userid == 409 || $userid == 137 || $userid == 390 || $userid == 562 || $userid == 582 ||
	//Marine
	($currentUser->IDempresa == 4 && $currentUser->role == "Enviar Registros") ||
	//GMT Prueba 2 semanas
	$userid == 276
	//Blumar
	|| $userid == 224 || $userid == 321 || $userid == 222
	//Bluriver
	|| $userid == 333
	//Yadran
	|| ($userid >=345 && $userid <=352)
	//Ventisqueros
	|| $userid == 423 || $userid == 425 ||  $userid == 468 ||  $userid == 604 ||  $userid == 605 || ($currentUser->IDempresa == 11 && $currentUser->role == "Enviar Registros")
	//Austral
	|| $userid == 490 || $userid == 491 || $userid == 492
	//Antartica
	|| $userid == 520 || $userid == 543 || $userid == 544
  //Si es administrador
  || $currentUser->role == "admin_fan_empresa"
	 ...
	 }else if($currentUser->role == "Laboratorio" || $currentUser->role == "usuario" || $currentUser->role == "Enviar Registros"){
        redirect('../index.php');
                            ...
                            <div id="graficosseccion4" style="margin-top:20px;">
                                ...
                                <div class="row" style="margin-left:10px; margin-right:10px;">
                                    ...
                                    <div class="" style="width: 100%; float: left; margin-top: 15px; margin-left:50px;">
                                        ...
                                        <select id="acsselectmap" name="multipleselectsearch" class="form-control no-print" style="float: left; max-width:80px; margin-right:10px;">
                                        </select>
                            ...
                            <div id="graficosseccion1" style="margin-top:20px;">
                                ...
                                <div class="row" style="margin-left:10px; margin-right:10px;">
                                    ...
                                    <div style="width: 80%; float: left; margin-top: 15px; margin-left:50px;">
                                        ...
                                        <select id='especie_filtro1' class="especie_filtro" name="multipleselect_usuario"  style=" margin-left:10px;cursor:pointer;" >
                                        </select>
...
function graficar01(btn){
	...
	if(btn){
		$.ajax({
				...
				data: { user_id: user_id, fecha_filtro_1: fecha_filtro_1, fecha_filtro_2:fecha_filtro_2, centros_filtro: centros  },
...
&& context.dataset.data[context.dataIndex] > 0;
		...
		title: {
			display: true,
			text: ' Cantidad de Ingresos Registrados por Centro '+ $('#fecha_filtro_1_01').val()+ " al " + $('#fecha_filtro_2_01').val()
		},
		scales: {
			 xAxes : [{
				  gridLines : {
...
function load_resumen_tabla(myobj){
		var rowCount = $('#resumenreporte th').length;
			...
			var html = $('#especie_02 option:selected' ).text() + " que Superan " + $('#valor_condicion_02').val() + " " + $('#condicion_02 option:selected' ).text() + unidad_aux;
...
&& context.dataset.data[context.dataIndex] > 0;
		...
		title: {
			display: true,
			text: 'Tendencia especie '+$('#especie_filtro1 option:selected').text()+' por centro',
			lineHeight: 1,
		},
		tooltips: {
			mode: 'index',
				...
				if(msg){
					...
					$.each(msg['Especies'], function (i, item) {
						...
						if($('#idespecie').val() != "" && $('#idespecie').val() == item['IDespecie']){
							$('#especie_filtro1').append('<option value="'+item['IDespecie']+'" selected>'+item['Nombre']+'</option>');
						}else{
